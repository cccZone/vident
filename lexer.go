...
func (self *Lexer) createToken(token_type int) {
     token := &Token {} 
     token.token_type = token_type
     token.content = self.flushBuffer()
...
func (self *Lexer) createLexer(input string) {
     self.input = input
     self.pos = 0
     self.input_length = utf8.RuneCountInString(self.input)
     self.line_number = 1
     self.char_number = 1
     self.start_pos = 0
...
func (self *Lexer) consumeCharacter() {
     if (self.pos + self.skipped_chars) - 1 > self.input_length {
        self.running = false
     }     
...
func (self *Lexer) recognizeCharacterToken() {
     ...
     self.createToken(CHARACTER)
}

func (self *Lexer) recognizeOperatorToken() {
     self.consumeCharacter()
     self.createToken(OPERATOR)
}
